			...
			case MOTOR_TYPE_RM3508:
				{
					...
					MotorSend(Joint[i].Rm3508_motor.hcan, Joint[i].Rm3508_motor.ID | 0x200, rm_data);
				}
				break;
			case MOTOR_TYPE_M2006:
				{
					int16_t current_output = (int16_t)((Joint[i].vel_pid.pid_out * enable_Joint[i]) +
						(Joint[i].exp_torque * enable_feedforward[i]));
					int16_t rm_data[4] = {current_output, 0, 0, 0};
					MotorSend(Joint[i].M2006_motor.hcan, Joint[i].M2006_motor.ID | 0x200, rm_data);
				}
				break;
			}
			if(i==2)
	...
	for (;;)
  {
		for (uint8_t i = 0; i < 5; i++)
	{
		...
		case MOTOR_TYPE_RS:
			...
			break;
		case MOTOR_TYPE_RM3508:
			raw_rad   = (Joint[i].Rm3508_motor.motor.Angle_DEG * 0.0174533f - Joint[i].pos_offset) * Joint[i].inv_motor;
			raw_omega = Joint[i].Rm3508_motor.motor.Speed * 0.0174533f * Joint[i].inv_motor;
			raw_torque = Joint[i].Rm3508_motor.motor.TorqueCurrent * 0.01f * Joint[i].inv_motor;
			break;
		case MOTOR_TYPE_M2006:
			raw_rad   = (Joint[i].M2006_motor.motor.Angle_DEG * 0.0174533f - Joint[i].pos_offset) * Joint[i].inv_motor;
			raw_omega = Joint[i].M2006_motor.motor.Speed * 0.0174533f * Joint[i].inv_motor;
			raw_torque = 0;
			break;
		}
